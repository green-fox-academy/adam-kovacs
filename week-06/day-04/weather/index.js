...
function getCity (cityList, req) {
  let i;
  for (i = 0; i <  cityList.length; i++) {
    if (cityList[i].city == req.params.cityID) {
      return cityList[i];
    }
  }
}
...
app.get('/', (req, res) => {
  res.render('dashboard', { cityData: forecasts });
});

app.get('/cities/:cityID', (req, res) => {
  let cityObject = getCity(forecasts, req);
  res.render('details', { cityData: cityObject });
});
